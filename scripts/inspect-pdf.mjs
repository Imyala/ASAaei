...
import { buildCells, detectPageFields } from '../src/pdfGrid.js'
...
for (let p = 1; p <= pdf.numPages; p++) {
  ...
  for (const f of fields) {
    console.log(`  ${f.type.padEnd(9)} x=${r(f.xPct * vp.width).toString().padStart(6)} y=${r(f.yPct * vp.height).toString().padStart(6)} w=${r(f.wPct * vp.width).toString().padStart(6)} h=${r(f.hPct * vp.height).toString().padStart(5)}  ${JSON.stringify(f.label)}`)
  }
  if (rawPage === p) {
